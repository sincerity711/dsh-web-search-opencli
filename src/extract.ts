...
export function extractionToDocument(extraction: GoogleAiModeExtraction): GoogleAiModeSearchDocument {
  const markdownWithMarkers = htmlToMarkdown(extraction.html ?? '')
  return embedCitations(markdownWithMarkers, extraction.citations ?? [])
}
...
function normalizeSource(source: GoogleAiModeRawSource): WebSearchSource | undefined {
  if (!source.url.startsWith('http://') && !source.url.startsWith('https://')) return undefined
